...
/// Parse go build diagnostic lines from stderr.
///
/// Format:
///   # package-path
...
///   FILE:LINE: message
///
/// Go compiler only emits errors (no warnings).
fn parse_go_diagnostics(stderr: &str) -> Vec<Diagnostic> {
    let main_re = Regex::new(r"^(.+?):(\d+):(?:(\d+):)?\s+(.+)$").unwrap();
...
fn try_main() -> Result<i32, (i32, String)> {
    ...
    if args.len() < 3 {
        return Err((
            1,
            "usage: judge-go-compile <diagnostics-file> <go args...>".to_string(),
        ));
    }
    ...
    let diagnostics = parse_go_diagnostics(&captured_text);
